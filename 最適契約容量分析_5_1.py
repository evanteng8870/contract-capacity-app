...
def build_pdf_report(
    *,
    ...
    # helper：DataFrame 轉 Table，數字保留一位小數，對齊處理
    def dataframe_to_table(df: pd.DataFrame, title: str | None = None) -> None:
    ...
    # 契約容量掃描結果
    dataframe_to_table(df_scan, "五、契約容量掃描結果（節錄）")
...
# ================== 12 個月最大需量輸入區 + 起算年月（右側） ==================
st.subheader("12 個月最大需量輸入")

# 起算年月：以今天月份往前推三年內的「年月轉軸」，放在這裡，靠右主畫面
today = datetime.date.today()
...
st.markdown(
    f"""
    <script>
    const keys = {json.dumps(month_input_keys)};
    ...
    setTimeout(setNumericInputMode, 100);
    </script>
    """,
    unsafe_allow_html=True,
)
